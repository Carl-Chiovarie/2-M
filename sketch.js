...
function genColor (variantColorBase, HSBVariPrefs){
...
let colorVariance = prompt("Input Color variance. Less is calm more is chaos. Recommended ~40")
  ...
  constructor (position, domColor, sideLen, hexWidth, ID){
    ...
    this.genHSBVariPrefs = function(){
      ...
      this.HSBVariPrefs = [
        ...
        -saturation,saturation, // not sure on these last 4 or really any of these
    ...
    this.genVari = function() {
      let newVari = genColor(this.domColor, this.HSBVariPrefs);
...
function genAllPixelBlocks() {
  ...
  if (confirm("Would you like to use custom inputs? Press cancel for default")){
    ...
    strokeWeight(prompt("Outline thickness"));
  } else {
...
let domTimeMinMax = [5, 12] // in seconds
let domSetTime;
